...
export default function DrawerMenu() {
    ...
    return (
        <>
            {/* Botão Hamburguer — z-[2000] garante ficar acima do Leaflet */}
            <button
                onClick={() => setOpen(true)}
                style={{ zIndex: 2000 }}
                className="fixed top-4 left-4 bg-[#7300E6] hover:bg-[#4B0099] text-white p-3 rounded-xl shadow-lg transition"
            >
                ☰
            </button>
            ...
            <aside
                ...
                className={`fixed top-0 left-0 h-full w-72 bg-gradient-to-b from-[#7300E6] to-[#4B0099]
                    shadow-2xl p-6 transition-transform duration-300 transform
            ...
            >
                ...
                <nav className="flex flex-col gap-4 text-lg text-white">
                    ...
                    <Link href="/endereco" onClick={() => setOpen(false)}
                        className="flex items-center gap-3 font-semibold hover:bg-[#A566FF33] px-3 py-2 rounded-lg transition">
                        🧭 Buscar por Endereço
                    </Link>
                    <Link href="/proximo" onClick={() => setOpen(false)}
                        className="flex items-center gap-3 font-semibold hover:bg-[#A566FF33] px-3 py-2 rounded-lg transition">
                        📍 Próximo a mim
                    </Link>
