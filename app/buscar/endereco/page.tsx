...
type SiteNear = {
  id: number; sigla: string; nome: string|null; endereco: string|null; lat: number|null; lon: number|null; distancia_m: number;
};
...
export default function BuscarPorEnderecoPage() {
  ...
  const handleSearch = async (address: string) => {
    setError(null); setSites([]); setResolved(null);
    if (!address) return;
    setLoading(true);
    try {
      ...
      const nearRes = await fetch(`/api/sites/near?lat=${geo.lat}&lng=${geo.lng}&radius=5000`, { cache: "no-store" });
    ...
    <section className="py-8 max-w-3xl mx-auto">
      ...
      {resolved && (
        <p className="mt-3 text-sm text-gray-600">
          Endereço resolvido: <span className="font-medium">{resolved.label}</span> — {resolved.lat.toFixed(6)}, {resolved.lng.toFixed(6)}
        </p>
      )}
      {loading && <p className="mt-4 text-sm text-gray-600">Carregando...</p>}
      {error && <p className="mt-4 text-sm text-red-600">{error}</p>}
      {!loading && !error && sites.length === 0 && <p className="mt-4 text-sm text-gray-600">Digite um endereço para iniciar.</p>}
      <div className="mt-4 grid gap-3">{sites.map((s) => <SiteCard key={s.id} site={s} />)}</div>
    </section>
  );
}